...
import { ENTITY_TYPES } from "@/lib/content/constants";

const DISPLAY_TYPES: EntityType[] = [
  "artist",
  "album",
  "track",
  "genre",
...
export function SearchResults() {
  ...
  const initialType = params.get("type") as EntityType | null;

  const [query, setQuery] = useState(initialQuery);
  const [typeFilter, setTypeFilter] = useState<EntityType | null>(initialType);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const rawDocuments: SearchDocument[] = Array.isArray(raw)
          ? raw
          : raw.documents ?? [];
    ...
    <div>
      ...
      {query.trim().length >= 2 && results.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 mb-8 pb-6 border-b border-white/[0.05]">
          ...
          {DISPLAY_TYPES.filter((t) => (counts[t] ?? 0) > 0).map((t) => {
            const color = ENTITY_TYPE_COLORS[t];
            return (
              <button
                key={t}
              ...
              >
                {ENTITY_TYPE_LABELS[t]} ({counts[t] ?? 0})
              </button>
            );
          })}
        </div>
