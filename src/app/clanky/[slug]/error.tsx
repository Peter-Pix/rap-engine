...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error('[article-detail] error:', error) }, [error])
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 py-20 text-center">
      <div className="glass rounded-2xl p-12 max-w-lg mx-auto">
        ...
        <p className="text-zinc-400 text-sm mb-6">Nepodařilo se načíst článek.</p>
        <div className="flex items-center justify-center gap-3">
          ...
          <a href="/clanky" className="px-4 py-2 rounded-lg bg-white/[0.06] ring-1 ring-white/10 text-zinc-300 text-sm">Všechny články →</a>
        </div>
      </div>
    </div>
  )
